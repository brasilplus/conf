...
function masterInstallFiles() {
    ...
    ${WGET_PATH} ${MIRROR}/solusvmphp7/install/${PATH_PREFIX}solusvm-bin-x86_64.tar.gz -O /usr/local/solusvm/tmp/solusvm-bin-x86_64.tar.gz >> ${INSTALL_LOG_LOCATION} 2>&1
    ${WGET_PATH} ${MIRROR}/solusvmphp7/install/${PATH_PREFIX}solusvm-bin-nvirt-x86_64.tar.gz -O /usr/local/solusvm/tmp/solusvm-bin-nvirt-x86_64.tar.gz >> ${INSTALL_LOG_LOCATION} 2>&1
